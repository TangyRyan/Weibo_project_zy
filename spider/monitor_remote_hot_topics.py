...
from update_posts import MAX_TOPICS_PER_RUN, POST_DIR, refresh_posts_for_date
from spider.daily_heat import update_daily_heat
POLL_INTERVAL_SECONDS = 600
RECENT_RETRY_SECONDS = 60
MAX_LOOKBACK_DAYS = 1
HOURLY_ARCHIVE_DIR = Path("../data/hot_topics/hourly")
LOG_LEVEL = logging.INFO
LOCAL_FALLBACK_THRESHOLD_MINUTES = 45
POST_EXPORT_DIR = Path("data/hot_posts")
...
def process_hour(date_str: str, hour: int) -> bool:
    ...
    _export_hourly_posts(date_str, hour, topics)
...
def _export_hourly_posts(date_str: str, hour: int, topics: List[dict]) -> None:
    hour_str = f"{hour:02d}"
    ...
    for topic in topics:
        ...
        try:
            ...
            logging.error("Failed to load post cache %s: %s", post_path, exc)
            continue
